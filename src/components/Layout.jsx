...
export default function Layout() {
  ...
  const navLink = (to, label) => ({
    ...
    transition:'all .15s',
    color: loc.pathname === to ? '#00c8f0' : '#6b7d99',
    background: loc.pathname === to ? '#00c8f018' : 'transparent',
    border: loc.pathname === to ? '1px solid #00c8f044' : '1px solid transparent',
  })
  ...
  return (
    <div style={{display:'grid',gridTemplateRows:'48px 1fr',height:'100vh'}}>
      <header style={{display:'flex',alignItems:'center',gap:14,padding:'0 20px',
        ...
        <Link to="/" style={{fontSize:15,fontWeight:700,letterSpacing:'.1em',color:'#00c8f0',textDecoration:'none'}}>
          PIT<em style={{color:'#6b7d99',fontStyle:'normal',fontWeight:500}}>DATA</em>
        </Link>
        <nav style={{display:'flex',gap:6,marginLeft:8}}>
          <Link to="/" style={navLink('/','Sesiones')}>Sesiones</Link>
          <Link to="/live" style={{...navLink('/live','Live'),
            color: loc.pathname==='/live' ? '#00e676' : '#6b7d99',
            background: loc.pathname==='/live' ? '#00e67618' : 'transparent',
            border: loc.pathname==='/live' ? '1px solid #00e67644' : '1px solid transparent',
          }}>
